...
Like the previous project, a SortedList will be able to store any type of data, be it integers, doubles, Strings, or objects of most any class.  There will only be one restriction: since the objects have to be kept in sorted order, there has to be some way to compare them.  This is straightforward for simple things like integers and doubles, but what if we made a `SortedList<Dog>`?  How do we determine if one `Dog` should come before or after another `Dog` in sorted order?  Java has a built-in mechanism for this.
...
`Comparable` only has one method that has to be implemented: `int compareTo(other)`. This method's job is to compare "this" object against another object (`other`) to see which one is bigger or smaller.  If this object is smaller, this function should return a negative integer (often `-1`).  If this object is bigger, it should return a positive integer (often `+1`).  If the two objects are equal, it should return `0`.
...
`SortedList`s have a few operations you must implement:
...
- You must implement all the methods in SortedList that are not already implemented. I suggest going in this order:
  ...
  - **Write `remove()` and testing code**.  This function should not crash if the item is not in the list; instead it should do nothing.
...
- Rules in video form: [https://www.youtube.com/watch?v=Lw2ua5B0TTM](https://www.youtube.com/watch?v=Lw2ua5B0TTM)
...
- **Write a function to determine the "card score"** for a player (scoring the cards in their hand at the end of the game)
- **Write an artificial intelligence player to play the game**.  If you would like, you may also enter your No Thanks AI player in a contest against other players from our class.
...
- **Here's what you must do**: **Make a new class that implements Player.**  The name of your class should be `SomethingSomethingPlayer` where the `SomethingSomething` can be whatever you want, but try to make it unique enough that someone else in the class won't come up with the same idea.  For example,  name your player after yourself, your pet, or just pick some random adjectives (SuperCrazyFunPlayer?). (Don't make your player names inappropriate.)
...
To submit, upload these three files: **SortedList.java, SortedListTester.java,** and **your AI player class** to canvas, including **your name and honor pledge**, and **ensure that your tests for all the required functions**.  Make sure to also submit your **answers to the post-project questions**
